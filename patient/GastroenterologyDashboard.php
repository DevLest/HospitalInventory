<?php
session_name('GastroenterologySession');  // Ensure session name matches the one set during login
...
if (!isset($_SESSION['doctor_id']) || $_SESSION['specialties'] != 'Gastroenterology') {
    // Redirect to login if the user is not a Gastroenterology doctor or not logged in
    header("Location: login.php?error=Access denied. Please log in as a Gastroenterology doctor.");
    exit();
}

// Your Gastroenterology-specific content goes here
echo "Welcome, Dr. " . $_SESSION['name'] . " (Gastroenterology Specialist)";
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="top-nav">
    ...
    <!-- Side Navigation -->
     <div id="mySidenav" class="side-nav">
        ...
        <a class="nav-link" href="GastroenterologyDashboard.php">
            <div class="sb-nav-link-content">
                <div class="sb-nav-link-icon"><i class="fas fa-home"></i></div>
                <span>𝖣𝖺𝗌𝗁𝖻𝗈𝖺𝗋𝖽</span>
            </div>
        </a>
        <a class="nav-link" href="GastroenterologyList.php">
            <div class="sb-nav-link-content">
                <div class="sb-nav-link-icon"><i class="fas fa-user"></i></div>
                <span>𝖯𝖺𝗍𝗂𝖾𝗇𝗍 𝖫𝗂𝗌𝗍</span>
            </div>
        </a>
        <a class="nav-link" href="GastroenterologySchedule.php">
            <div class="sb-nav-link-content">
                <div class="sb-nav-link-icon"><i class="fas fa-user"></i></div>
                <span>Schedule</span>
            </div>
    ...
    <div class="row"> <!-- Use row class to create a horizontal group of columns -->
       <div class="col-xl-3 col-md-6">
        ...
        <?php
                ...
                // Define the specialty you want to filter by (e.g., Gastroenterology)
                $specialty = 'Gastroenterology';  // Example specialty

                // Query to fetch all appointments for doctors with the "Gastroenterology" specialty
                $dash_category_query = "
                ...
                if ($appointments_total = mysqli_num_rows($dash_category_query_run)) {
                    ...
                    echo '<h4 class="mb-0" style="z-index: 2; position: relative;">No Data for Gastroenterology</h4>';
                }
        ...
        // Query to fetch all pending appointments for doctors with the "Gastroenterology" specialty
...
?>
      ...
      <div class="col-xl-3 col-md-6">
        <div class="card text-white mb-4" style="background-color: #DCC7AA; opacity: 0.9; box-shadow: 0 2px 5px rgba(0, 0, 0, 1.9);">
            ...
            // Query to fetch all approved appointments for doctors with the "Gastroenterology" specialty
       ...
       <div class="col-xl-3 col-md-6">
        <div class="card text-white mb-4" style="background-color: #DCC7AA; opacity: 0.9; box-shadow: 0 2px 5px rgba(0, 0, 0, 1.9);">
            ...
            // Query to fetch the total number of appointments for doctors with the "Gastroenterology" specialty
...
// Ensure the user is logged in and is a Gastroenterology doctor
if (!isset($_SESSION['doctor_id']) || $_SESSION['specialties'] != 'Gastroenterology') {
    header("Location: login.php?error=Access denied. Please log in as a Gastroenterology doctor.");
    exit();
}
