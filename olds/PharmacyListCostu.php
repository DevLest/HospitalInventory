...
// Query to fetch data from pharmacycustomers table
$queryCustomers = "SELECT * FROM pharmacycustomers";
$resultCustomers = mysqli_query($db, $queryCustomers);

// Query to fetch data from pharmacydoctors table
$queryDoctors = "SELECT * FROM pharmacydoctors";
$resultDoctors = mysqli_query($db, $queryDoctors);

// Check if both queries executed successfully
if (!$resultCustomers || !$resultDoctors) {
...
<div id="layoutSidenav_content">
        <main>  
            <div>
                <h1 style="margin-left: 3%; margin-top: 1%;"><i class="fas fa-handshake"></i>     Manage Costumers</h1>
                <p style="font-weight: bold; margin-left: 8%;">Manage Existing Costumer</p>
            </div>
            <div  style="opacity: 0.9; border-top: 2px solid black;"><br>
    ...
    <tbody>
        <?php
        // Loop through both result sets simultaneously to combine data from both tables
        while (($rowCustomer = mysqli_fetch_assoc($resultCustomers)) && ($rowDoctor = mysqli_fetch_assoc($resultDoctors))) {
        ?>
            <tr>
                <td><?php echo $rowCustomer['customer_id']; ?></td>
                <td><?php echo $rowCustomer['customer_name']; ?></td>
                <td><?php echo $rowCustomer['contact_number']; ?></td>
                <td><?php echo $rowCustomer['address']; ?></td>
                <td><?php echo $rowDoctor['doctor_name']; ?></td>
                <td><?php echo $rowDoctor['doctor_address']; ?></td>
                                                    <td>
                                   <div class="button-container">
                                    <form method="post" action="edit.php">
                                        <input type="hidden" name="id" value="<?php echo $row['id']; ?>">
                                        <button type="submit" class="btn btn-primary btn-sm" style="background-color: blue;"> <i style='font-size:20px; color: white;' class='fas'>&#xf044;</i></button>
                                    </form>
                                </div>
                                </td>
