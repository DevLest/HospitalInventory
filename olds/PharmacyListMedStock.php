...
<div class="row mb-3">
    ...
    <div class="col-md-2">
        <button type="button" class="btn btn-danger" style="font-weight: bold; margin-left: 30%;" id="outOfStockBtn" onclick="filterOutOfStock()">Out of Stock</button>
    </div>
    <div class="col-md-2">
        <button type="button" class="btn btn-warning" style="font-weight: bold;" id="expiredBtn" onclick="filterExpired()">Expired</button>
    </div>
</div>
    ...
    $(document).ready(function () {
        ...
        function filterTable() {
            ...
            $('#datatablesSimple tbody tr').each(function () {
                ...
                var rowSupplier = $(this).find('td:eq(5)').text().toLowerCase();

                // Show or hide table row based on search criteria
                if (rowMedicineName.includes(medicineName) && rowSupplier.includes(supplier)) {
                    $(this).show();
                } else {
                    $(this).hide();
                }
        ...
        // Function to filter out of stock items based on packing
        function filterOutOfStock() {
            $('#datatablesSimple tbody tr').each(function () {
                var packing = $(this).find('td:eq(2)').text().toLowerCase();
                var packingValue = parseInt(packing.replace(/\D/g, '')); // Extract numeric value from packing
        ...
        // Function to filter expired items
        function filterExpired() {
            ...
            $('#datatablesSimple tbody tr').each(function () {
                var expiryDate = $(this).find('td:eq(4)').text();
                var [month, year] = expiryDate.split('/').map(Number);

                // Show rows where the expiry date is this year and this month or earlier
        ...
        // Call filterOutOfStock function when clicking on out of stock button
        $('#outOfStockBtn').click(filterOutOfStock);

        // Call filterExpired function when clicking on expired button
        $('#expiredBtn').click(filterExpired);
    });
</script>
