...
<article id="printableArticle" style="box-shadow: 0px 0px 10px rgba(0, 0, 0, 0.5); height: 67%; width: 67%;">
    <div class="hello" style="background-color: #D5D8DC; height: 50px;">
        <h1 style="font-size: 25px; text-align: left; color: green; margin-bottom: 40px; padding: 10px;">
            <span style="margin-right: 280px;">ᴏᴘᴅ ꜰɪɴᴅɪɴɢꜱ</span>
            <a href="OutpatientView.php?id=<?php echo $patient['id']; ?>">
                <i id="back" style="color: black;" class="fas fa-arrow-left"></i>
            </a>
            <a href="opdFindingsEdit.php?patient_id=<?php echo $patient['id']; ?>">
                <i id="edit" style="color: black; margin-left: 30px;" class="fas fa-edit"></i>
            </a>
            <a href="javascript:void(0);" onclick="printArticle()">
                <i id="printIcon" style="color: black; margin-left: 30px;" class="fas fa-print"></i>
            </a>
        </h1>
    </div>
        ...
        <div class="card-body">
            ...
            <div style="display: grid; grid-template-columns: repeat(2, 1fr);">
                ...
                if(isset($_GET['patient_id']) && isset($_GET['vital_signs_id'])) {
                    // Retrieve the patient's ID and vital signs ID from the URL parameters
                    $patient_id = $_GET['patient_id'];
                    $vital_signs_id = $_GET['vital_signs_id'];
                    ...
                    if(mysqli_num_rows($result_patient) > 0 && mysqli_num_rows($result_vital_signs) > 0) {
                        ...
                        ?>
                        <div>
                            <h1 style="font-size: 15px; margin-top: 20px; color: gray;">PATIENT NAME:</h1>
                            <h2 style="font-size: 25px;"><?php echo $patient['lastname'] . ', ' . $patient['firstname'] . ' ' . $patient['middlename']; ?></h2>

                            <h1 style="font-size: 15px; margin-top: 20px; color: gray;">HISTORY OF PATIENT ILLNESS:</h1>
                            <h2 style="font-size: 25px;"><?php echo !empty($vital_signs['history_of_patient_illness']) ? $vital_signs['history_of_patient_illness'] : 'N/A'; ?></h2>

                            <h1 style="font-size: 15px; margin-top: 20px; color: gray;">BLOOD PRESSURE:</h1>
                            <h2 style="font-size: 25px;"><?php echo !empty($vital_signs['blood_pressure']) ? $vital_signs['blood_pressure'] : 'N/A'; ?></h2>

                            <h1 style="font-size: 15px; margin-top: 20px; color: gray;">TEMPERATURE:</h1>
                            <h2 style="font-size: 25px;"><?php echo !empty($vital_signs['temperature']) ? $vital_signs['temperature'] : 'N/A'; ?></h2>

                            <h1 style="font-size: 15px; margin-top: 20px; color: gray;">PHYSICIAN EXAMINATION:</h1>
                            <h2 style="font-size: 25px;"><?php echo !empty($vital_signs['physical_examination']) ? $vital_signs['physical_examination'] : 'N/A'; ?></h2>

                            <h1 style="font-size: 15px; margin-top: 20px; color: gray;">DIAGNOSIS:</h1>
                            <h2 style="font-size: 25px;"><?php echo !empty($vital_signs['diagnosis']) ? $vital_signs['diagnosis'] : 'N/A'; ?></h2>

                            <h1 style="font-size: 15px; margin-top: 20px; color: gray;">MEDICATION / TREATMENT:</h1>
                            <h2 style="font-size: 25px;"><?php echo !empty($vital_signs['medication_treatment']) ? $vital_signs['medication_treatment'] : 'N/A'; ?></h2>

                            <h1 style="font-size: 15px; margin-top: 20px; color: gray;">ATTENDING PHYSICIAN:</h1>
                            <h2 style="font-size: 25px;"><?php echo !empty($vital_signs['attending_physician']) ? $vital_signs['attending_physician'] : 'N/A'; ?></h2>
                        </div>

                        <div style="margin-left: 70px;">
                            <h1 style="font-size: 15px; margin-top: 20px; color: gray;">RESPIRATORY RATE:</h1>
                            <h2 style="font-size: 25px;"><?php echo !empty($vital_signs['respiratory_rate']) ? $vital_signs['respiratory_rate'] : 'N/A'; ?></h2>

                            <h1 style="font-size: 15px; margin-top: 20px; color: gray;">PULSE RATE:</h1>
                            <h2 style="font-size: 25px;"><?php echo !empty($vital_signs['pulse_rate']) ? $vital_signs['pulse_rate'] : 'N/A'; ?></h2>

                            <h1 style="font-size: 15px; margin-top: 20px; color: gray;">CAPILLARY REFIL:</h1>
                            <h2 style="font-size: 25px;"><?php echo !empty($vital_signs['capillary_refill']) ? $vital_signs['capillary_refill'] : 'N/A'; ?></h2>

                            <h1 style="font-size: 15px; margin-top: 20px; color: gray;">WEIGHT:</h1>
                            <h2 style="font-size: 25px;"><?php echo !empty($vital_signs['weight']) ? $vital_signs['weight'] : 'N/A'; ?></h2>

                            <h1 style="font-size: 15px; margin-top: 20px; color: gray;">DATE CONSULTED:</h1>
                            <h2 style="font-size: 25px;"><?php echo date('Y-m-d'); ?></h2>
                        </div>
                        <?php
                    } else {
                        echo "Patient not found or no vital signs data found for the specified ID.";
                    }
                }
                ?>
            </div>
...
<script>
    function printArticle() {
        // Itago ang mga icon ng print
        document.getElementById('printIcon').style.display = 'none';
         document.getElementById('back').style.display = 'none';
          document.getElementById('edit').style.display = 'none';

        // Kunin ang nilalaman ng article
        var printableArticle = document.getElementById('printableArticle').innerHTML;
        var originalContents = document.body.innerHTML;

        // Palitan ang nilalaman ng body sa loob ng article
        document.body.innerHTML = printableArticle;

        // Mag-print
        window.print();

        // Ibalik ang orihinal na nilalaman ng body
        document.body.innerHTML = originalContents;

        // Ibalik ang mga icon ng print
