...
<div class="card mt-4" style=" box-shadow: 0 2px 5px rgba(0, 0, 0, 0.9);">
    <div class="card-header" style="background-color: #34495e;">
        <h5 class="mb-0" style="color: white;"><i class="fas fa-user" style="color: white;"></i> 𝖯𝗁𝖺𝗋𝗆𝖺𝖼𝗒 𝖢𝖺𝗌𝗁𝗂𝖾𝗋 / 𝖯𝗁𝖺𝗋𝗆𝖺𝖼𝗒 𝖲𝗍𝖺𝖿𝖿 / 𝖶𝖺𝗋𝖽𝗌 / 𝖤𝖱 𝖭𝗎𝗋𝗌𝖾 ,<span style="margin-left: 20px;"> 𝖫𝗈𝗀𝗂𝗇/𝖫𝗈𝗀𝗈𝗎𝗍 𝖧𝗂𝗌𝗍𝗈𝗋𝗒</span></h5>
...
// SQL query to fetch combined data from users and pharmacy_cashier_cash_in tables
$sql = "SELECT 
            u.id AS user_id,
            u.username,
            u.shift,
            u.role,
            (c.one * 1 + c.five * 5 + c.ten * 10 + c.twenty * 20 +
                c.fifty * 50 + c.hundred * 100 + c.five_hundred * 500 +
                c.thousand * 1000) AS total_cash_in,
            DATE(c.cash_in_time) AS cash_in_date
        FROM 
            users u
        INNER JOIN 
            pharmacy_cashier_cash_in c ON u.id = c.user_id
        WHERE 
            u.role = 'Pharmacy Cashier' 
            AND u.username LIKE ?
        ORDER BY 
            cash_in_date DESC, u.username";

// Prepare and execute the query
$stmt = $conn->prepare($sql);
$searchTermWithWildcards = "%" . $searchTerm . "%"; // For partial matches
...
<div class="card mt-4" style="box-shadow: 0 2px 5px rgba(0, 0, 0, 0.9);">
    <div class="card-header" style="background-color:#34495e;">
        <h5 class="mb-0" style="color: white;">
            ...
            Pharmacy Cashier Cash-In History
        </h5>
        <form method="POST" class="form-inline mt-2">
            <input type="text" name="search" class="form-control mr-2" placeholder="Search Username" value="<?php echo htmlspecialchars($searchTerm); ?>">
            <button type="submit" class="btn btn-primary">Search</button>
        </form>
    </div>
    <div class="card-body">
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        echo "<tr>
                                <td>{$row['username']}</td>
                                <td>{$row['shift']}</td>";
                        // Format total cash-in
                        if ($row['total_cash_in'] !== null) {
                            echo "<td>₱ " . number_format($row['total_cash_in'], 2) . "</td>";
                        } else {
                            echo "<td>₱ 0.00</td>";
                        }
                        echo "<td>" . date("Y-m-d", strtotime($row['cash_in_date'])) . "</td>
                              </tr>";
                    }
                } else {
